...
void vShowBme280Values(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  display->clear();
  display->setTextAlignment(TEXT_ALIGN_LEFT);
  display->setFont(ArialMT_Plain_10);
  display->drawString(0, 5, "T: " + String(bme.readTemperature()) + " *C");
  display->drawString(0, 20, "P: " + String(bme.readPressure() / 100.0F) + " hPa");
  display->drawString(0, 35, "h: " + String(bme.readAltitude(SEALEVELPRESSURE_HPA)) + " m");
  display->drawString(0, 50, "H: " + String(bme.readHumidity()) + " %");
}

// This array keeps function pointers to all frames
// frames are the single views that slide in
FrameCallback frames[] = { analogClockFrame, digitalClockFrame, vShowVoltages,vShowBme280Values };
